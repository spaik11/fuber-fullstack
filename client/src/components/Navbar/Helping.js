                     ...
                     {friendList.map(friend=>{
                        if(friend.email !== userEmail && friend.requestHelpSent){
                           return(
                              ...
                              <li style={styledLi} onClick={()=> props.setActiveMarker(friend)}>
                                 <div>{friend.username}</div>
                                 <div>{distance(userLocation.lat, userLocation.lng, friend.lat, friend.lng, 'M').toFixed(1)} mi.</div>
                              </li>
                              </React.Fragment>
                           )
                        }else{
...
export default connect(mapStateToProps, { cancelHelp, setActiveMarker })(Helping)
